...
По результатам обследования выделены пять приоритетных областей, требующих немедленного решения:

### Управление доступом: 

* В личных кабинетах клиенты видят информацию других пользователей.
* Отсутствует централизованное управление доступом и контроль учётных записей.

### Безопасность данных:

* Нет контроля над персональными данными внутри компании и при обмене с партнёрами.

### Защита инфраструктуры: 

*  IT-ландшафт PropDevelopment активно развивался: появлялись новые интеграции с поставщиками услуг для продажи недвижимости и новые услуги для жильцов. Необходимо упорядочить инфраструктуру.

### Управление инцидентами: 

* Недавно у конкурента произошла утечка данных, которая активно обсуждалась в СМИ. После инцидента у конкурента риск стал неприемлемым.
...
| 4   | Реализован ли принцип минимальных привилегий при назначении ролей и прав доступа?                                                        | нет | контракты API партнеров позволяют взаимодействовать с данными других компаний |
...
| 8   | Используются ли системы аутентификации пользователей для доступа к системам?                                                             | да | два варианта - Keycloak и ActiveDirectory |
| 9   | Созданы ли замкнутые контуры для подсистем, работающих с чувствительными данными?                                                        | нет | данные свободно передаются между компонентами |
| 10  | Ограничен ли доступ к системам из внешних источников?                                                                                    | да | для клиентов настроен Firewall |
| 11  | Ведётся ли единый реестр учётных записей?                                                                                               | нет | данные расположены в отдельных компонентах |
| 12  | Используются ли средства управления учётными записями на базе единого реестра?                                                          | нет| данные расположены в отдельных компонентах |
| 13  | Применяются ли средства для отзыва прав доступа?                                                                                        |       |              |
| 14  | Используются ли средства централизованного контроля доступа?                                                                            | нет | данные расположены в отдельных компонентах |
...
| 12  | Реализованы ли системы или процессы контроля сохранности данных?                                                                        | да | для всех БД выполняется резервное копирование |
...
| 1   | Используется ли межсетевой экран для ограничения входящего и исходящего трафика?                                                        | да | для клиентов настроен Firewall |
...
| 6   | Как часто проводятся внутренние и внешние аудиты безопасности? Обновляются ли политики и процедуры на основе результатов этих аудитов?  | нет | компания готовится к этому мероприятию |
